//CITATION - https://bitbucket.org/andrewtwest/orbtrak/src/default/orbtrak.js
...
var Orb =
	{
		...
		{
			Orb.map = map;
			Orb.crossBrowserSetStyle(map, "background: url(../img/earth.jpg) no-repeat; overflow: hidden;", true);

			var frag = document.createDocumentFragment();
			var div = document.createElement("div");
			div.id = "home";
			Orb.crossBrowserSetStyle(div, "position:relative; width: 24px; height: 24px; background-image: url(../orbtrak/orbimages/home.gif);", false);
		...
		//modified this function to be responsive
		setHomeCoordinates: function(homeLat, homeLng)
		{
			PLib.configureGroundStation(homeLat, homeLng);


			var img = document.getElementById('map-div');

			var mapWidth = img.offsetWidth;
			var mapHeight = (mapWidth/2163.0) * 1082;

			Orb.home.style.left = (((mapWidth/360.0) * (180 + (parseInt(homeLng))))-12) + "px";
			Orb.home.style.top = (((mapHeight/180.0) * (90 - homeLat))-12) + "px";
		...
		createOneMarker: function(txt)
		{
			...
			Orb.crossBrowserSetStyle(div, "position:absolute; width: 24px; height: 24px; background-image: url(../orbtrak/orbimages/saticon.gif);", false);
		...
		updateSatellites: function()
		{
			...
			for (var i = 0; i < PLib.sat.length; i++)
			{
				...
				Orb.satelliteMarkers[i].style.left = (((mapWidth/360.0) * (180 + satInfo.longitude))-12) + "px";
				Orb.satelliteMarkers[i].style.top = (((mapHeight/180.0) * (90 - satInfo.latitude))-12) + "px";
